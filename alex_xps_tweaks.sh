...
# Install touchad gestures with Fusuma
gpasswd -a $USER input #MUST be a member of the INPUT group to read touchpad by Fusuma, MUST REBOOT to assign this group
apt install -y libinput-tools
apt install -y ruby
gem install -y fusuma
apt install -y xdotool
gsettings set org.gnome.desktop.peripherals.touchpad send-events enabled #Ensure the touchpad events are being sent to the GNOME desktop
mkdir -p ~/.config/fusuma        # create config directory
cp ~/Git/xpsinstall/touchpad.yaml ~/.config/fusuma
wget https://raw.githubusercontent.com/letourneaualex/xps #get startup config nofile
cp ~/Git/xpsinstall/fusuma.desktop .config/autostart
...
# Install Atom
wget -qO - https://packagecloud.io/AtomEditor/atom/gpgkey -P ~/Dowloads/ | apt-key add
sh -c 'echo "deb [arch=amd64] https://packagecloud.io/AtomEditor/atom/any/ any main" > /etc/apt/sources.list.d/atom.list'
apt update
apt install atom
apm install atom-beautify atom-ide-ui ide-yaml language-cisco language-lua minimap script

# Install Pip
apt install -y python3-pip

# Enable and Install Firewall
ufw enable
apt install gufw
...
wget https://www.realvnc.com/download/file/viewer.files/VNC-Viewer-6.19.1115-Linux-x64.deb -P ~/Downloads/
dpkg -i ~/Downloads/VNC-Viewer-6.19.1115-Linux-x64.deb

# Install Python IDLE
apt update
apt install -y idle-python3.7
...
wget -O ~/vmware.bin https://www.vmware.com/go/getplayer-linux -P ~/Downloads/
